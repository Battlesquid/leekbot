	...
	message: {
		...
		async locked({ message }) {
			try {
				...
				if (lockedChannels.includes(message.channel.id) && ((message.attachments.size < 0 && (message.content.length > 0 && !(new RegExp(process.env.URL_REGEX).test(message.content)))) || (message.content.length > 0 && !(new RegExp(process.env.URL_REGEX).test(message.content))))) {
					message.delete();
				}
			} catch (e) { console.log(e); }
		}
